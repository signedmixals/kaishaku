...
char *safe_path_join(const char *dir, const char *file) {
    ...
    if (snprintf(path_buffer, sizeof(path_buffer), "%s/%s", dir, file) >= (int)sizeof(path_buffer)) {
...
void usage(void) {
    printf("%skaishaku - a minimal tool for safe Git experimentation via detached HEAD sessions%s\n\n", 
    ...
    printf("  %skaishaku clean%s [<session>]             Remove session(s)\n", 
           COLOR_YELLOW, COLOR_RESET);
    printf("  %skaishaku exit%s [--force | --keep | --save | --no-save]  Exit session and return to original branch\n", 
           COLOR_YELLOW, COLOR_RESET);
...
int execute_git_command(const char *cmd, char *output, size_t output_size) {
    ...
    if (output && output_size > 0) {
        if (!fgets(output, output_size, fp)) {
            pclose(fp);
            snprintf(error_message, sizeof(error_message), 
                    "Command returned no output: %s", cmd);
            return 0;
        }
    ...
    if (status == -1 || WEXITSTATUS(status) != 0) {
        snprintf(error_message, sizeof(error_message), 
                "Command failed with status %d: %s", WEXITSTATUS(status), cmd);
        return 0;
    }
...
void cmd_checkout(const char *session, const char *commit) {
    ...
    if (!execute_git_command("git rev-parse --abbrev-ref HEAD", 
                           current_branch, sizeof(current_branch))) {
    ...
    if (!commit) {
        if (!execute_git_command("git rev-parse HEAD", 
                               head_hash, sizeof(head_hash))) {
            fprintf(stderr, "Error: %s\n", error_message);
            exit(EXIT_FAILURE);
        }
        commit = head_hash;
...
void cmd_branch(const char *branch_name) {
    ...
    printf("%sCreated branch '%s' from session '%s'%s\n", COLOR_GREEN, branch_name, session, COLOR_RESET);
}
...
void cmd_save(const char *branch_name) {
    ...
    printf("%sSuccessfully saved changes from session '%s' to branch '%s'%s\n", 
           COLOR_GREEN, session, original_branch, COLOR_RESET);
}
...
void cmd_exit(const char *option) {
    ...
    if (has_changes) {
        if (save) {
            ...
            snprintf(commit_msg, sizeof(commit_msg), 
                    "[kaishaku] Save changes from session '%s'", session);
            ...
            snprintf(stash_msg, sizeof(stash_msg), 
                    "kaishaku: auto-stash from session '%s'", session);
            
            char stash_cmd[DEFAULT_BUFFER_SIZE+23];
            snprintf(stash_cmd, sizeof(stash_cmd), 
                    "git stash push -m \"%s\"", stash_msg);
            ...
            if (!execute_git_command(stash_cmd, NULL, 0)) {
                ...
                printf("%sChanges stashed successfully. Use 'git stash list' to see your stashes.%s\n", 
                       COLOR_GREEN, COLOR_RESET);
            }
        } else {
    ...
    printf("%sReturned to branch '%s' from session '%s'%s\n", 
           COLOR_GREEN, original_branch, session, COLOR_RESET);
}
...
void cmd_list(void) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (!file_exists(session_file) || !file_exists(head_file)) {
            ...
            fprintf(stderr, "%sWarning: Session '%s' appears to be corrupted. Use 'recover' to fix.%s\n",
        ...
        if (original_branch) {
            snprintf(branch_check, sizeof(branch_check), 
                    "git rev-parse --verify %s", original_branch);
            branch_exists = execute_git_command(branch_check, NULL, 0);
        }
        
        if (head) {
            snprintf(commit_check, sizeof(commit_check), 
                    "git rev-parse --verify %s", head);
...
void cmd_config(int argc, char *argv[]) {
    ...
    if (strcmp(cmd, "get") == 0) {
        ...
        snprintf(git_cmd, sizeof(git_cmd), 
                "git config --local kaishaku.%s %d", key, bool_value);
    ...
    if (!execute_git_command(branch_check, NULL, 0)) {
        ...
        if (!execute_git_command(create_branch, NULL, 0)) {
            fprintf(stderr, "%sError: Failed to create branch '%s'.%s\n", 
                    COLOR_RED, original_branch, COLOR_RESET);
            exit(EXIT_FAILURE);
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "checkout") == 0 && argc >= 3) {
        ...
        const char *commit = (argc >= 4) ? argv[3] : NULL;
        cmd_checkout(session, commit);
        ...
        const char *option = (argc == 3) ? argv[2] : NULL;
        cmd_exit(option);
        ...
        const char *session = (argc == 3) ? argv[2] : NULL;
        cmd_clean(session);
